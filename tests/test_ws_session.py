...
def _digit_events(n, is_dog=True, subject="golden retriever"):
    """`n` `report_verdict(...)` calls, as the model would emit them."""
    from google.adk.events import Event
    from google.genai import types
...
def _nudges(spy):
    return [t for t in spy.texts if "Stop calling report_verdict" in t]
...
def test_a_repeat_run_is_nudged_once(main_module, monkeypatch, ws_connect, spy):
    ...
    assert _nudges(spy) == [
        "golden retriever is recorded. Stop calling report_verdict and say the "
        "confirmation now."
    ]

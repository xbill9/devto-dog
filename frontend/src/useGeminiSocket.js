...
export function useGeminiSocket(
  url,
  { onVerdict, onSystemError, onHeavyMetal, onDropped } = {},
) {
  ...
  // Held in a ref as well as returned: the wake-word listener is built
  // inside startStream and would otherwise close over a stale copy.
  const requestScanRef = useRef(null);
    ...
    (overrideUrl) => {
      ...
      ws.current.onmessage = async (event) => {
        ...
        if (msg.type === "pong") {
          ...
          //
          // `is_dog` is a boolean, so every truthiness check here has to be an
          // explicit undefined test: `msg.is_dog || ...` would discard every
          // NOT-A-DOG verdict, which is half the answers and the more
          // interesting half.
          if (msg.type === "match") {
            if (msg.is_dog !== undefined) {
              const verdict = {
                isDog: Boolean(msg.is_dog),
                subject: msg.subject || "unidentified",
                confidence:
              ...
              console.log(
                `[DEBUG] MATCH SIGNAL FROM BACKEND: ${
                  verdict.isDog ? "DOG" : "NOT A DOG"
                } (${verdict.subject})`
              );
              if (onVerdictRef.current) onVerdictRef.current(verdict);
              pushEvent(
                "match",
                `${verdict.isDog ? "dog" : "not a dog"} — ${verdict.subject}`
              );
            }
          ...
          if (parts.length > 0) {
            ...
            parts.forEach((part) => {
              ...
              // every detection fire onVerdict twice, because the
  ...
  const startStream = useCallback(
    async (videoElement) => {
      try {
        ...
        // The scheduler is built regardless of mic mode. It used to live inside
        // the wake-word branch, which meant the click-to-scan button had
        // nothing to call anywhere SpeechRecognition is missing -- i.e. exactly
        // the browsers the button exists for.
        ...
        if (micMode === "wake" && wakeWordSupported()) {
          try {
            wakeWord.current = new WakeWordListener(undefined, {
              onCommand: (heard) => {
                // Same path the button takes, so the two can never drift.
                requestScanRef.current?.(`"${heard}"`);
              },
            });
  ...
  /**
   * Ask for a scan. Called by the wake word and by the button, so there is
   * exactly one definition of what "scan" does.
   *
   * `source` is for the event log only. Frames go first: they share a socket
  ...
  const requestScan = useCallback((source = "button") => {
    ...
    pushEvent(
      "you",
      `${source} \u2192 2 frames + scan${outcome === "held" ? " (queued, model speaking)" : ""}`,
    );
    return outcome;
  }, [pushEvent]);
  useEffect(() => {
    requestScanRef.current = requestScan;
  }, [requestScan]);

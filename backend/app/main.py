...
# Close a session nobody is using. Seconds since the last SCAN REQUEST, not
# since the last frame -- the camera streams ~128 kbit/s of video the whole
# time it is open, so `last_input_time` never goes stale and would never
# reap anything.
#
# This exists because the 65-second round timer was removed with the
# finger-counting handshake, and that timer was also the only thing that
# ever ended a billed session. Without it an abandoned tab streams video
# until the browser closes. 0 disables it.
IDLE_TIMEOUT_S = max(0.0, float(os.getenv("IDLE_TIMEOUT_S", "180")))
...
#
# That last paragraph used to end "do not trade video resolution for bandwidth:
# the uplink is ~77% microphone, so the savings are in the audio gate, not
# here". **That is no longer true.** The audio gate was taken -- the microphone
# stays in the browser now (wakeWord.js) -- so video is essentially the entire
# uplink, and this is the only remaining lever on it.
#
# Which reopens the question rather than settling it. The table above was
# measured on fixtures where the subject filled the frame; a dog held up on a
# phone does not. Re-measure with `scan_accuracy.py --matrix` against the dog
# fixtures before lowering this, and keep accuracy as the deciding number.
# Video is buying accuracy; bandwidth is what it costs.
...
# On which `report_verdict` call of a single scan the backend asks for the spoken
...
# the Live preview model by coincidence -- it is a half-cascade model,
...
async def websocket_endpoint(
    ...
    # How many `report_verdict` calls have arrived since the last turn we put to
    ...
    if language_code == "en-US":
        ...
        opening = (
            ...
            f"including every verdict and the containment breach announcement. "
            f'Say the {language_name} for "Scanner Online." and nothing else.'
        )
    ...
    async def upstream_task() -> None:
        ...
        frame_count = 0
        audio_count = 0
        nonlocal last_input_time, last_scan_time, input_sample_rate
    ...
    last_match_verdict = None
    last_match_time = 0
    last_input_time = asyncio.get_event_loop().time()
    last_scan_time = last_input_time
    ...
    async def idle_reaper_task() -> None:
        """Close the session when nobody has asked for a scan in a while.

        Cost control, not a transport concern: the Live session is billed for
        every second it is open and the camera is uploading the whole time. A
        demo left open in a tab is the most expensive thing this app can do.

        Deliberately generous -- someone talking through a demo can go three
        minutes between scans, and closing under a presenter is worse than the
        bandwidth it saves. Set IDLE_TIMEOUT_S=0 to disable.
        """
        if IDLE_TIMEOUT_S <= 0:
            logger.info("Idle reaper disabled")
            return
        try:
            while True:
                await asyncio.sleep(5.0)
                idle = asyncio.get_event_loop().time() - last_scan_time
                if idle > IDLE_TIMEOUT_S:
                    logger.info(
                        f"Idle {idle:.0f}s with no scan request; closing session"
                    )
    ...
    async def downstream_task() -> None:
        ...
        model_audio_count = 0
        nonlocal last_match_verdict, last_match_time
        ...
        async for event in runner.run_live(
            ...
            for fc in function_calls:
                ...
                if fc.name == "report_verdict":
                    is_dog = fc.args.get("is_dog")
                    if is_dog is not None:
                        subject = fc.args.get("subject") or "unidentified"
                        confidence = fc.args.get("confidence")
                        current_time = asyncio.get_event_loop().time()
                        # Improved deduplication: 1.5s window OR different verdict.
                        # Keyed on is_dog rather than the subject string, matching
                        # agent.py: "golden retriever" and "a dog, golden retriever"
                        # are the same answer arriving twice.
                        if (
                            is_dog != last_match_verdict
                            or (current_time - last_match_time) >= 1.5
                        ):
                            last_match_verdict = is_dog
                            ...
                            logger.info(
                                f"Sending MATCH signal to frontend: "
                                f"{'DOG' if is_dog else 'NOT A DOG'} ({subject})"
                            )
                        ...
                        if (
                            ...
                            send_text_stimulus(
                                live_request_queue,
                                f"{subject} is recorded. Stop calling "
                                f"report_verdict and say the confirmation now.",
                            )
                elif fc.name == "trigger_system_error":
                    logger.warning("SYSTEM ERROR TRIGGERED BY MODEL")
                    error_msg = {
                        "type": "system_error",
                        "message": "CRITICAL PROTOCOL VIOLATION: FELINE DETECTED. SCANNER INTEGRITY LOST.",
                    }
                    ...
                    # The frame type is still `heavy_metal`: renaming it would
                    # touch the socket contract, the mock and the tests for a
                    # string no user ever sees. What changed is what it means.
    ...
    helpers = [
        ...
        asyncio.create_task(idle_reaper_task(), name="idle-reaper"),
    ]
...
# The eval fixtures, for the phone portal (`/portal.html`).
#
# Serving test assets from the app is deliberate: the portal exists so a phone
# held up to the webcam cycles the SAME images the accuracy harness scores, and
# a second copy would drift from the first the day someone added a fixture.
#
# Mounted before the SPA catch-all below, which would otherwise swallow it.
...
async def list_fixtures(reveal: bool = False) -> dict:
    """The fixture set, for the portal.

    **Ground truth is withheld unless explicitly asked for.** The portal renders
    these images full-bleed into a camera's field of view, so any label on the
    page is a label the model can read -- and a scanner that scores 100% because
    it read "grey wolf" off the screen has measured nothing at all. `reveal=1`
    is the presenter's toggle, and the portal never requests it while a scan can
    be in flight.
    """

...
### I let an agent build a live-video scanner over a weekend. The interesting part is not that it worked.
...
It is deliberately not charming about it. The scanner is a cold threat-assessment system that happens to have been pointed at dogs, and the entire joke is that it does not know it is making one. Hold up a golden retriever and it says, in the flattest voice available: *"Woof."* Hold up a wolf and it says *"Negative. Grey wolf."* Hold up a cat and it suffers a fatal system error. Get three dogs in frame and it declares a containment breach — in whichever of nine languages you picked.
...
![One scan, end to end: a phone showing a dog photo held up to the webcam, a green DOG verdict reading "small dog 98%", and the telemetry panel showing detect 492 ms, think 490 ms, speak 871 ms.](https://raw.githubusercontent.com/xbill9/devto-dog/5ca6791aafc94a2864ba76252da0dd4c903506a9/posts/dog.gif)

Live at [dog-or-not.run.app](https://dog-or-not-289270257791.us-central1.run.app); source at [github.com/xbill9/devto-dog](https://github.com/xbill9/devto-dog).
...
### Clean lint over a failed build

Deleting a dead function took a live one out with it, four lines below. ESLint passed — it does not resolve cross-module imports by default — and the failure only appeared in `vite build`, buried under twelve lines of rollup stack trace.

### A green test asserting the exact thing it was supposed to prevent

The project keeps a non-public model id out of the repo, enforced by a gate wired into the test and deploy targets. One test asserted that id as a string literal. A leak with a checkmark on it.

And the gate itself failed on its first run — on its own list of patterns, which necessarily contains every string it searches for. Funny once, an infinite loop thereafter.
...
It sees the dog. It narrates the call. **Zero tool calls, ever.**

Meanwhile the fallback model inherited from an earlier build could not open a Live session at all, under a comment claiming it could — because until the first real session, nothing had ever opened one.
...
It drives the real deployed endpoint with fixture images at 640×480 JPEG q60 — the exact format the browser sends — puts a text frame in where you would say "scan", and scores every `report_verdict` against a count verified by eye before it was committed. `--blur-prob` and `--jitter` approximate a real webcam; `--min-rate` turns it into a gate that can fail a build.
...
- **17 correct verdicts** — six breeds, wolves, coyotes, foxes, two bronze statues
- **1 containment breach** — correct; three dogs in frame
- **2 cat alarms** — both cats triggered the system error
- **0 non-dogs called dogs**
- **0 real dogs refused**

Latency 0.68–1.56 s, mostly around 0.7. It correctly said *"bronze statue"* for a sculpture of a man with a bronze dog beside him.
...
It optimised what it could reach instead of what mattered, and reported a blocker it had not verified. That is a very human failure mode and it is worth naming, because the honest version of "I let an agent build this over a weekend" includes it.

---

## What it runs on

**Gemini Live through the Agent Development Kit.** Bidirectional WebSocket streaming video at 1 FPS, structured verdicts through tool calls rather than parsed prose, and a nine-language session config where the model translates its own confirmations rather than reading a shipped phrasebook. That last part is the cheapest possible proof the model is really being called: a recording cannot answer in Japanese.

**The bark pack is generated at build time, never at runtime.** The clips are fetched and decoded once and held in memory, so the bark adds zero latency to the response path and cannot fail during a session. This project had already measured what a second audio stream does to a Live session — 0/5 against 5/5 — and the right way to use a sound API here was to make the app touch the network *less*, not more.

Sound effects generated with [ElevenLabs](https://elevenlabs.io). Fixture images from Wikimedia Commons, attributed in the repo.

---

*Measured on `gemini-3.1-flash-live-preview` via `google-adk 2.6.3` and `google-genai 2.17.0`, Python 3.13, deployed to Cloud Run in us-central1 with a 3600s request timeout — a WebSocket is one long request, and the default 300s cap would end every session at five minutes. Video 640×480 JPEG q60 at 1 FPS, audio 16 kHz PCM up and 24 kHz down. Tested in Chrome; the wake word is `SpeechRecognition`, which is Chrome and Edge only, and everywhere else an on-screen button does the identical thing.*
